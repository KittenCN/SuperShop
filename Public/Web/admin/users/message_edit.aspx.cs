...
namespace Web.admin.users
{
    public partial class message_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == AXEnums.ActionEnum.View.ToString())
            {
                this.action = AXEnums.ActionEnum.View.ToString();//修改类型
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (action == AXEnums.ActionEnum.View.ToString()) //修改
                {
                    ShowInfo(this.id);
                }
            }
        ...
        {
            ChkAdminLevel("user_message", AXEnums.ActionEnum.Add.ToString()); //检查权限
            if (!DoAdd())
            {
                JscriptMsg("发送过程中发生错误！", "", "Error");
                return;
            }
            AddAdminLog(AXEnums.ActionEnum.Add.ToString(), "发送站内短消息"); //记录日志
