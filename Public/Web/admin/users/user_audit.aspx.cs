...
namespace Web.admin.users
{
    public partial class user_audit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_audit", AXEnums.ActionEnum.View.ToString()); //检查权限
        ...
        {
            ChkAdminLevel("user_audit", AXEnums.ActionEnum.Audit.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.users bll = new BLL.users();
            ...
            AddAdminLog(AXEnums.ActionEnum.Audit.ToString(), "审核用户成功" + sucCount + "条，失败" + errorCount + "条"); //记录日志
