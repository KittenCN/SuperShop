...
namespace Web.admin.channel
{
    public partial class channel_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_channel_list", AXEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定类别
                FieldBind(); //绑定扩展字段
        ...
        private string GetInherit(string page_type)
        {
            ...
            {
                case "index":
                    result = "Web.UI.Page.article";
                    break;
                case "category":
                    result = "Web.UI.Page.category";
                    break;
                case "list":
                    result = "Web.UI.Page.article_list";
                    break;
                case "detail":
                    result = "Web.UI.Page.article_show";
                    break;
            }
            return result;
        }
        ...
        private bool DoAdd()
        {
            ...
            AddAdminLog(AXEnums.ActionEnum.Add.ToString(), "添加频道" + model.title); //记录日志
            return true;
        }
        #endregion
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("site_channel_list", AXEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("site_channel_list", AXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
