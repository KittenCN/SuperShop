...
namespace Web.admin.order
{
    public partial class express_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_express", AXEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private bool DoAdd()
        {
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(AXEnums.ActionEnum.Add.ToString(), "添加配送小区:" + model.title); //记录日志
                return true;
            }
            return false;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(AXEnums.ActionEnum.Edit.ToString(), "修改配送小区:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == AXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ...
                JscriptMsg("修改配送小区成功！", "express_list.aspx", "Success");
            }
            else //添加
            {
                ChkAdminLevel("order_express", AXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
                }
                JscriptMsg("添加配送小区成功！", "express_list.aspx", "Success");
            }
        }
    }
}