...
namespace BLL
{
	...
	public partial class channel_category
	{
        ...
        public bool Exists(string build_path)
        {
            ...
            if (DirPathExists(siteConfig.webpath + "/" + AXKeys.DIRECTORY_REWRITE_ASPX + "/", build_path))
            {
                return true;
            }
            //与频道名称是否同名
		...
		public bool Update(Model.channel_category model)
		{
            ...
            if (dal.Update(model))
            {
                ...
                {
                    //更改频道分类对应的目录名称
                    Utils.MoveDirectory(siteConfig.webpath + AXKeys.DIRECTORY_REWRITE_ASPX + "/" + oldModel.build_path,
                        siteConfig.webpath + AXKeys.DIRECTORY_REWRITE_ASPX + "/" + model.build_path);
                    Utils.MoveDirectory(siteConfig.webpath + AXKeys.DIRECTORY_REWRITE_HTML + "/" + oldModel.build_path,
                        siteConfig.webpath + AXKeys.DIRECTORY_REWRITE_HTML + "/" + model.build_path);
                }
                //修改对应的导航
        ...
        {
            List<Model.channel_category> modelList = new List<Model.channel_category>();
            ...
            if (rowsCount > 0)
            {
                ...
                {
                    model = new Model.channel_category();
