...
namespace Web.admin.order
{
    public partial class order_config : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_config", AXEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        {
            ChkAdminLevel("order_config", AXEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.orderconfig bll = new BLL.orderconfig();
            Model.orderconfig model = bll.loadConfig();
            try
            {
                ...
                AddAdminLog(AXEnums.ActionEnum.Edit.ToString(), "修改订单配置信息"); //记录日志
